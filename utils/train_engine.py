...
def train_engine(__C, dataset, dataset_eval=None):
    ...
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (
            ...
            frcn_feat_iter2 = frcn_feat_iter2.cuda()
            grid_feat_iter = grid_feat_iter.cuda()
            bbox_feat_iter = bbox_feat_iter.cuda()
            ques_ix_iter = ques_ix_iter.cuda()
            ans_iter = ans_iter.cuda()
            caption_ix_iter = caption_ix_iter.cuda()
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                sub_frcn_feat_iter2 = \
                    frcn_feat_iter2[accu_step * __C.SUB_BATCH_SIZE:
                                   (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_grid_feat_iter = \
                ...
                sub_ans_iter = \
                    ans_iter[accu_step * __C.SUB_BATCH_SIZE:
                             (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_caption_ix_iter = \
                    caption_ix_iter[accu_step * __C.SUB_BATCH_SIZE:
                             (accu_step + 1) * __C.SUB_BATCH_SIZE]
