...
class SGA_Rel(nn.Module):
    def __init__(self, __C):
        super(SGA_Rel, self).__init__()

        self.mhatt1 = RelMHAtt(__C)
        self.mhatt2 = MHAtt(__C)
        self.ffn = FFN(__C)
    ...
    def forward(self, x, y, x_mask, y_mask, rel_embed):
        #todo RSA modify   has finished
        x = self.norm1(x + self.dropout1(
            self.mhatt1(v=x, k=x, q=x, mask=x_mask, rel_embed=rel_embed)
        ))
...
class MCA_ED(nn.Module):
    def __init__(self, __C):
        ...
        self.dec_list = nn.ModuleList([SGA_Rel(__C) for _ in range(__C.LAYER)])

    def forward(self, y, x, y_mask, x_mask,rel_embed):
        ...
        for dec in self.dec_list:
            x = dec(x, y, x_mask, y_mask,rel_embed)

        return y, x


class MCA_caption(nn.Module):
    def __init__(self, __C):
        super(MCA_caption, self).__init__()
