...
class DataSet(BaseDataSet):
    def __init__(self, __C):
        ...
        captions_BLIP_path = '/media/yanfeng/WD1500/vqa_caption_BLIP/captions_BLIP_VQA.json'
        # 从文件中加载json格式数据，将文件中的数据转换为字典类型(dict)
        self.captions_BLIP_dict = json.load(open(os.path.join(captions_BLIP_path), 'r'))

        captions_OFA_path = '/media/yanfeng/WD1500/vqa_caption_OFA/captions_OFA_VQA.json'
        self.captions_OFA_dict = json.load(open(os.path.join(captions_OFA_path), 'r'))
    ...
    def tokenize(self, stat_ques_list, use_glove):
        ...
        #todo
        # load = np.load('/media/yanfeng/9801/vqa_caption_BLIP/voc.npz')['voc']
        # print(f"caption voc size:{load.size}")
        # voc = load.tolist()
        voc_dir2 = '/media/yanfeng/WD1500/vqa_caption_BLIP/voc_BLIP_and_OFA.json'
    ...
    def load_ques_ans(self, idx):
        ...
        if self.__C.RUN_MODE in ['train']:
            ...
            caption_name = (frcn_feat_path).split('/')[-1].replace('.npz', '')
            caption = str(self.captions_OFA_dict[caption_name])
            caption_ix_iter = self.proc_ques2(caption, self.token_to_ix, max_token=14)


            return ques_ix_iter, ans_iter, iid, caption_ix_iter
            ...
            return ques_ix_iter, np.zeros(1), iid, caption_ix_iter
    ...
    def load_img_feats(self, idx, iid):
        ...
        try:
            frcn_feat_path2 = self.iid_to_frcn_feat_path[iid].replace("region_152_offcial_fpn",
                                                                      "region_152_offcial_from_grid_with_bbox_from_fpn")
            frcn_feat2 = np.load(frcn_feat_path2)
            frcn_feat_x2 = frcn_feat2['x']
            frcn_feat_iter2 = self.proc_img_feat(frcn_feat_x2,
                                                 img_feat_pad_size=self.__C.FEAT_SIZE['vqa']['FRCN_FEAT_SIZE'][0])
        except:
            print()
            print(frcn_feat_path2)
            print()
            frcn_feat_iter2 = frcn_feat_iter
        ...
        return frcn_feat_iter, frcn_feat_iter2, grid_feat_iter, bbox_feat_iter
