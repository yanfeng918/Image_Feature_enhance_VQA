...
class BaseDataSet(Data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ques_ix_iter, ans_iter, iid, caption_ix_iter = self.load_ques_ans(idx)

        frcn_feat_iter,frcn_feat_iter2, grid_feat_iter, bbox_feat_iter = self.load_img_feats(idx, iid)

        return \
            torch.from_numpy(frcn_feat_iter),\
            torch.from_numpy(frcn_feat_iter2),\
            torch.from_numpy(grid_feat_iter),\
            torch.from_numpy(bbox_feat_iter),\
            torch.from_numpy(ques_ix_iter),\
            torch.from_numpy(ans_iter),\
            torch.from_numpy(caption_ix_iter)
